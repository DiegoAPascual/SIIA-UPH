<?php
class RecursosModel extends Query{
    ...
    //Subir archivos
    public function subirarchivo($nombre, $tipo, $id_carperta, $id_usuario)
    {
       $sql = "INSERT INTO archivos (nombre, tipo, id_carpeta, id_usuario) VALUES (?, ?, ?, ?)";
       $datos = array($nombre, $tipo, $id_carperta, $id_usuario);
       return $this->insertar($sql, $datos);
    }
    ...
    public function getArchivos($id_carperta, $id_usuario) {
        $sql = "SELECT a.* FROM archivos a INNER JOIN carpetas c ON a.id_carpeta = c.id WHERE a.id_carpeta = $id_carperta AND c.id_usuario = $id_usuario ORDER BY a.id DESC";
        return $this->selectAll($sql);
    }

    public function getArchivosCompartidos($id_carpeta) 
    {
        $sql = "SELECT d.id, d.correo, d.estado, d.fecha_eliminacion, a.nombre FROM detalle_archivos d INNER JOIN archivos a ON d.id_archivo = a.id INNER JOIN carpetas c ON a.id_carpeta = c.id WHERE a.id_carpeta = $id_carpeta;";
        return $this->selectAll($sql);
    }
