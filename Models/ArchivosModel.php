...
class ArchivosModel extends Query{
    ...
    {
        $sql = "SELECT * FROM carpetas WHERE id_usuario = $id_usuario AND estado = 1 AND id != 1 ORDER BY id DESC LIMIT $desde, $porPagina";
        return $this->selectAll($sql);
    }
    ...
    {
        $sql = "UPDATE detalle_archivos SET estado = ?, fecha_eliminacion = ? WHERE id = ?";
        $array = [0, $fecha, $id];
    ...
    {
        $sql = "UPDATE archivos SET estado = ?, fecha_eliminacion = ? WHERE id = ?";
